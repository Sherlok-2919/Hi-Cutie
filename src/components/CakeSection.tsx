...
export default function CakeSection({ onCakeCut, cakeCut }: CakeSectionProps) {
  ...
  const endCut = (e?: React.PointerEvent<HTMLDivElement>) => {
    ...
    // require only a small minimum path so short drags still count
    if (cutPoints.length < 4) {
      setCutPoints([]);
      return;
    }

    // once a valid cut is drawn, reveal the split cake
      ...
      >
        ...
        >
          ...
          {!cakeCut && (
            ...
            >
              Click and drag your knife across the cake to cut it! 🎂
            </motion.div>
          )}
        </div>
